...
class DetailsIdeasBoxAccountingView extends StatelessWidget {
  final int ideaId;
  const DetailsIdeasBoxAccountingView({super.key, required this.ideaId});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => DetailsIdeasBoxCubit()..fetchDetailsIdeasBox(ideaId),
      child: const _DetailsIdeasBoxAccountingBody(),
    );
  }
}

class _DetailsIdeasBoxAccountingBody extends StatefulWidget {
  const _DetailsIdeasBoxAccountingBody();

  @override
  State<_DetailsIdeasBoxAccountingBody> createState() =>
      _DetailsIdeasBoxAccountingBodyState();
}

class _DetailsIdeasBoxAccountingBodyState
    extends State<_DetailsIdeasBoxAccountingBody> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<DetailsIdeasBoxCubit, DetailsIdeasBoxState>(
      builder: (context, state) {
        ...
        final idea = isLoading ? DetailsIdeasBoxModel.fake() : state.data;
        ...
        final safePage =
            totalPages == 0 ? 0 : _commentPage.clamp(0, totalPages - 1);
            ...
            body: SingleChildScrollView(
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    InfoRow(label: loc.idea_title, value: idea?.title ?? ''),
                    SizedBox(height: 16.h),
                    InfoRow(label: loc.author, value: idea?.authorName ?? ''),
                    SizedBox(height: 16.h),
                    InfoRow(
                    ...
                    SizedBox(height: 16.h),
                    InfoRow(
                        label: loc.status, value: idea?.statusLabel ?? ''),
                    SizedBox(height: 16.h),
                    InfoRow(
                        label: loc.votes, value: '${idea?.votesCount ?? 0}'),
                                ...
                                Row(
                                  ...
                                  children: [
                                    Text(
                                      comment.authorName ?? '',
                                      style:
                                          MyTextStyle().textStyleSemiBold14(),
                                    ),
                                    Text(
                                      comment.createdAt ?? '',
                                      style: MyTextStyle().textStyleMedium14(),
