import 'package:hb/core/config/constants.dart';
import 'package:hb/core/config/storage/remote_dio.dart';
import 'package:hb/core/data/models/country_model.dart';
import 'package:dio/dio.dart' as dio;

abstract class CountryRemoteDataSource {
  Future<List<Country>> getCountryCodes();
}

class CountryRemoteDataSourceImpl implements CountryRemoteDataSource {
  static final CountryRemoteDataSourceImpl _instance = CountryRemoteDataSourceImpl._internal();
  late final RemoteConnectionDio _remoteConnectionDio;

  /// Private constructor for singleton pattern
  CountryRemoteDataSourceImpl._internal() {
    _remoteConnectionDio = RemoteConnectionDio();
  }

  /// Factory constructor to return singleton instance
  factory CountryRemoteDataSourceImpl() {
    return _instance;
  }

  @override
  Future<List<Country>> getCountryCodes() async {
    try {
      final response = await _remoteConnectionDio.dio.get(Constants.countryCodesUrl);

      if (_isSuccessfulResponse(response)) {
        // Replace 'data' with the actual key from your API response
        final List<dynamic> countriesData = response.data['data'];
        return countriesData.map((json) => Country.fromJson(json)).toList();
      } else {
        throw Exception("Failed to load country codes: Status ${response.statusCode}");
      }
    } on dio.DioException catch (e) {
      throw Exception('Network error while fetching country codes: ${e.message}');
    } catch (e) {
      throw Exception('Unexpected error while fetching country codes: $e');
    }
  }

  bool _isSuccessfulResponse(dio.Response response) {
    return response.statusCode == 200;
  }
}
