...
class ChatCubit extends Cubit<ChatState> {
  ...
  void fetchChats({String? search}) async {
    _lastSearch = search;
    emit(state.copyWith(isLoading: true, errorMessage: null));
    try {
      final fetchedChatData =
          await UseCases.getChatRepositoryUsecase(search: search);
      emit(state.copyWith(chatData: fetchedChatData, isLoading: false));
  ...
  Future<int?> _resolveUserId() async {
    ...
    try {
      ...
      log('Chat list: resolve userId failed: $e');
    }
    return _userId;
  }
  ...
  void markConversationRead(int conversationId) {
    ...
    final list = current
        .map((c) => c.id == conversationId && c.unreadCount > 0
            ...
            : c)
        .toList();
    emit(state.copyWith(chatData: list));
  }
  ...
  Future<void> _refreshSilently() async {
    ...
    _refreshing = true;
    try {
      final data = await UseCases.getChatRepositoryUsecase(search: _lastSearch);
      emit(state.copyWith(chatData: data));
    } catch (e) {
      log('Chat list silent refresh failed: $e');
    } finally {
      _refreshing = false;
      if (_pendingRefresh) {
