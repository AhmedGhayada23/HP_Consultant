...
class HbLabIdeasBoxAccountingCubit
    extends Cubit<HbLabIdeasBoxAccountingState> {
  HbLabIdeasBoxAccountingCubit() : super(HbLabIdeasBoxAccountingState());
  ...
  Future<void> fetchHbLabIdeasBox({
    ...
    try {
      final result = await UseCases.getHbLabIdeasBoxAccountingUsecase(
        search: _lastSearch,
        page: 1,
  ...
  Future<void> toggleUpvote(int id) async {
    try {
      final result = await UseCases.getHbLabUpvoteUsecase(id);
